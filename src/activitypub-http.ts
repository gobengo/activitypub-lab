...
import type { ActivityPub as IActivityPub } from "./service";
...
import { AnnounceActivityPubDotComActivity } from "./activitypub-server";
...
async function GetDidResolution(activitypubService: IActivityPub) {
  ...
  return AsyncRequestListener(async function (
    ...
    const inboxPostInvocation = Client.invoke({
      issuer: httpServerIssuer,
      audience: activitypubService,
      capability: {
        can: "activitypub/inbox/post",
        with: did,
        body: AnnounceActivityPubDotComActivity(),
      },
    });
    ...
    const resolution = await inboxPostInvocation.execute(connection);
    if (!resolution.ok) {
